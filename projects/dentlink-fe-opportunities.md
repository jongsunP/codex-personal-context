...
## 현재 기준 — 2026-09-18 최종 회의 정리

- **선택 과제는 하나:** 실제 덴트링크 컴포넌트를 기반으로 AI가 우리 UI에 맞는 화면을 만들고, PM·디자이너와 FE가 그 결과를 함께 활용하는 환경을 만듭니다. **DLDS 정비 → AI 프롬프트·스킬·작업 흐름 정비(핵심) → 필요하고 가능하면 덴트링크 에디터 개발** 순서입니다.
...
- 현재 완료된 일은 **회의 결과 정리·기록**입니다. 새 제품 코드·브랜치·PR·배포나 AI 구현·품질 검증 실험은 진행하지 않았습니다. 실제 구현은 다음 범위 구체화와 사용자 지시를 따릅니다.
...
- 실제 FE 컴포넌트와 프로젝트·디자인 규칙을 AI가 읽고 자연어 요구를 화면과 코드로 연결하도록 합니다. 프롬프트뿐 아니라 스킬, 사용할 도구, 질문·구현·검증 순서까지 다듬는 과정이며 반복 검증에 가장 많은 시간이 들 것으로 봅니다.
- **목표:** 매번 Figma 링크를 주지 않아도 실제 컴포넌트와 정리된 기준으로 Figma에 가까운 UI가 나오는 것. 화면의 목적·내용·동작 등 필요한 요구는 입력받아야 하며, 필요한 정보가 빠지면 재질문합니다. 전달하지 않은 화면 배치까지 추론해 일치한다고 약속하지 않습니다.
- **예상 흐름:** 자연어 요청 → 필요한 정보 재질문 → 실제 컴포넌트로 화면 구현 → 미리보기·검증 → FE 검토·제품 기능 연결. PM·디자이너도 결과를 확인하고 FE가 실제 제품 코드에 활용할 수 있어야 합니다. 제품 데이터·권한·업무 동작이 자동 완성됐다는 뜻은 아닙니다.
- 디자인 일치도·코드 품질·반복 일관성과 FE의 UI 재작성·수정 왕복 감소는 앞으로 실제 화면으로 검증합니다. 아직 성공률·작업 절감 효과·소요 기간을 측정하지 않았습니다.

### 3. 회의 메모 연결과 에디터 확장

- **PM·디자이너도 FE 코드 기반:** 실제 제품 컴포넌트로 기획·디자인 결과를 확인하고 FE가 화면과 코드를 이어받는 목적입니다. 이들이 직접 로컬 저장소나 개발 서버를 운영해야 한다는 확정 요구는 아닙니다.
- **스킬·하네스:** AI가 읽을 자료, 사용할 컴포넌트·도구, 질문·구현·검증 절차를 정합니다. 하네스는 이 절차를 실행하고 확인하는 환경을 뜻합니다.
...
2. 대표 AI 요청·화면과 최소 입력, 디자인·코드 검증 및 실제 FE 작업 절감 확인 방법.
