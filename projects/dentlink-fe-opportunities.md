...
- 최신 구성은 **업무 목록 표 1개(10개 업무) → 표와 같은 순서의 업무별 설명**입니다. 후속 요청으로 회의 메모와 바로 연결되는 업무명, 설명, 예상 작업일을 표에 두고 결과물·FE 범위·조건은 아래로 옮겼습니다. QR의 두 흐름은 한 업무 안에 유지했고 파일 탭·어드민 주문상세 개선을 추가했습니다.
- 표현 기준: `예상 인일` 대신 **예상 작업일**, FE 1명 기준을 짧게 설명하고 기간은 **8~13일** 형식을 사용합니다. 반복 배경·콜아웃·별도 회의 진행안·긴 코드 근거 목록은 덜어냈습니다. 글 분량은 약 40% 줄였고 도식 2개는 원본 블록 그대로 유지했습니다. 표에는 첫 결과물보다 업무의 의미를 먼저 보여줍니다.
...
- 공수에는 중복 합산 금지와 기존 API/권한 재사용 가정을 명시했습니다. 확장 에디터의 추가 12~20일은 범위 미선정으로 팀 문서에서는 제거하고 첫 검증 후 재산정으로 정리했습니다. 앞선 개인 추정치를 확정 견적으로 되살리지 않습니다.
...
- 후속 간소화 검증: 표 10개 업무와 설명 10개, 범위 `~`, 기존 이미지 저장 경로 2개 보존, 구 용어·회의 진행안 제거를 재조회했습니다. 브라우저에서 표 전체·기간 표기·축약 설명·QR 도식 표시를 확인했습니다. 새 두 안건은 첫 범위를 고른 뒤 작업일을 산정하도록 표시했으며 임의의 기간을 추가하지 않았습니다.
