...
- 사용자 확인 목표: **실제 덴트링크 컴포넌트를 기반으로 AI가 우리 UI에 맞는 화면을 만들고, PM·디자이너와 FE가 그 결과를 함께 활용하는 환경을 만듭니다.** 과제는 하나이며 **DLDS(v1) 정비 → AI 프롬프트 정비가 핵심 과정이고, 에디터는 이후 가능한 확장**입니다. 이 결론을 기준으로 사용자와 논의·구체화하며 진행합니다.
...
- 기존 데모앱: https://dentlink-experience-studio.parkjongsunfrankie.chatgpt.site . 2차 자료에는 새 데모/사이트를 만들지 않았습니다. 도식 2개는 작성 중 사용했다가 제거했으며, 최종본은 텍스트 리스트입니다.
- 이 체크포인트는 개인 작업 재개용입니다. 팀에는 위 노션 링크만 제공합니다.
- **선택 과제는 아래 2차 회의 결과 한 가지입니다.** 기존 후보·병행 목록을 이번에 모두 실행하는 것으로 해석하지 않습니다. 과제 선택은 새 제품 코드·브랜치·PR 착수 승인이 아닙니다.
- **현재 Notion은 회의 전 검토안입니다.** 메인과 작은 병행 업무를 나눈 기존 내용은 유지 중이며 새 회의 결과는 아직 Notion에 반영하지 않았습니다. 사용자는 이번에 Git 기반 메모리에 결론과 후속 논의 항목을 정리하도록 요청했습니다.

## 아이데이션 2차 팀 회의 결론 · 사용자 확인 — 2026-09-18

- **확정 결론:** 여러 안건 중 하나만 진행합니다. **디자인시스템 정비 → AI 프롬프트 깎기 → 가능하면 에디터 개발** 순서이며, AI 프롬프트 단계가 핵심이고 오래 걸릴 것으로 봅니다. AI가 실제 덴트링크 컴포넌트로 우리 UI에 맞는 화면을 만들고 PM·디자이너와 FE가 결과를 함께 활용하는 환경을 만드는 것이 사용자 확인 목표입니다. 에디터 완성 자체를 필수 최종 성과로 고정하지 않습니다.
- **버전 명칭과 코드 근거 구분:** 사용자는 전달받은 요구와 사례를 **DLDS=v1, 공유한 DLOS 페이지=v2**로 이해했으나, 자신도 정비 요구와 링크를 전달받은 입장이라 다른 관계일 가능성이 있는지 후속 질문했습니다. 이 명칭은 현재 회의의 작업상 구분으로 사용하고 **공식 패키지 버전·전환 관계가 검증됐다고 기록하지 않습니다.** 제품의 `@dentlink/ui`에는 이미 일부 v2 export가 있고 갤러리 예시는 `@dentlink/dlos`여서 코드 경로·버전의 일대일 대응은 별도 확인이 필요합니다. 이전 ‘버전 관계 확정·재질문하지 않음’은 근거보다 강한 표현이므로 이 기록으로 정정합니다. **기존 DLDS를 정비하고 DLOS의 정리 사례를 참고한다**는 작업 방향은 유지하며, 첫 단계에서 참고 범위와 적용 기준을 확인합니다.
- **참조 확인:** https://dentlink-dlos.vercel.app/#component-text-field 를 브라우저에서 확인했습니다. DLOS Component Gallery에는 Figma Core 토큰, 컴포넌트 상태·상호작용·반응형 미리보기, Code/Props/Tokens 상세가 있고 사용 예시는 `@dentlink/dlos` import입니다. 이를 v1 정리의 참고 사례로 사용하되 같은 화면·구조를 그대로 복제해야 한다는 뜻은 아닙니다. 개별 소스 경로와 실제 적용 범위는 작업 구체화 때 확인합니다.
- **메모의 해석:** FE 실제 컴포넌트와 프로젝트 규칙을 AI가 읽고 자연어 요구를 구현·미리보기로 연결하는 환경을 만들려는 방향입니다. 필요한 입력이 없으면 질문하고, 허용된 UI·코드 사용 범위를 벗어나지 않게 스킬/실행·검증 절차를 정비한다는 뜻으로 읽었습니다. 목표 사용자는 PM·디자이너까지 포함하며, 로컬 소스를 쓴다는 말이 이들이 직접 저장소·개발 서버를 다뤄야 한다는 확정 요구는 아닙니다. AI가 만든 화면과 코드를 FE가 실제 이어받을 수 있어야 한다는 기존 목표는 유지됩니다.
- **부가 아이디어:** `/design/`, `/publish/`는 제작·미리보기용 페이지 경로 예시로 이해하며 확정 경로나 실서비스 배포 기능으로 보지 않습니다. 요소 선택 → 관련 컴포넌트·코드 위치 연결 → 그 부분에 대한 프롬프트 수정은 이후 편집 경험의 후보입니다. ‘Orca’의 구체적인 도구 지칭과 구현 방식은 미확인입니다. Storybook은 대체 가능성을 열어 둔 의견이며 즉시 삭제·폐기 승인으로 보지 않습니다. 전체 컴포넌트를 볼 카탈로그는 필요하다는 요구입니다.
- **진행 원칙:** 최초 전체 아이디어 → 1차 팀 회의 메모 → 2차 회의 검토 자료 → 오늘 확정 결론의 이력을 보존합니다. 기존 후보와 작은 병행 목록은 검토 이력이며 이번에 모두 실행할 목록이 아닙니다. 아래 미정 항목은 사용자와 앞으로 논의하고 작업하면서 발전시킵니다. 지금 전부 재질문하거나 미확정 방식을 확정 설계로 기록하지 않습니다.
- **이전안과의 관계:** 이전 ‘목록 열·필터 설정 JSON 편집기’는 이제 선택 과제의 확정 구조가 아닙니다. 기존 12~19일은 그 제한된 검증안의 추정이므로 AI 프롬프트 반복 검증이 중심인 새 과제의 견적으로 재사용하지 않습니다. 새로운 범위·기간은 별도 구체화가 필요합니다.
- **읽기 전용 코드 확인:** web checkout은 다른 작업의 `feature/amplitude-pageview-tracking`, HEAD `de2ffdd9e3025cb758632788cd6086c170e4974e`, `clinic/src/common/hooks/useAmplitudeInit.ts` 수정 상태였습니다. 이 저장소는 pull·checkout·수정하지 않았습니다. `shared/ui/src/index.ts`에 DLDS와 일부 v2 export가 공존하고 `lab/src/components/OrderApproval/OrderApprovalAcceptModal.tsx`는 기존 TextInput을 사용합니다. 기존/v2 InputBase의 기본값 차이와 기존 Storybook 예시는 확인했으나 차이를 곧바로 결함이나 전환 정책으로 단정하지 않습니다. 문서·제품·Sites는 수정하지 않았고 새 구현·검증 실험도 하지 않았습니다.

### 이어서 구체화할 항목

1. **v1 정비 범위와 기준:** 기존 DLDS 코드 정비가 대상인지, DLOS의 규칙 채택·전환까지 포함되는지 경계를 먼저 확인합니다. Figma·현재 서비스·코드 중 불일치를 맞출 기준과 실제 컴포넌트·사용 화면·props 변경 영향을 정합니다. 다음 논의는 이 범위와 정리 완료 기준부터 시작하며, 전체 구현 방식의 사전 확정을 요구하지 않습니다.
2. **컴포넌트 확인 페이지:** v2 사례 중 v1에도 필요한 코드 예시·props·토큰·상태별 보기의 범위와 제공 위치를 정합니다. Storybook의 유지·대체 시점은 별도 판단하며 즉시 삭제하지 않습니다.
3. **AI 입력·작업 방식:** AI가 읽을 실제 코드·규칙·예시, 자연어 요청의 최소 정보, 정보가 부족할 때의 재질문, 스킬·실행·검증 절차를 구체화합니다. FE와 PM·디자이너가 어디에서 사용하고 결과를 어떤 형태로 이어받을지도 정해야 합니다.
4. **품질과 효과 기준:** Figma와 맞춘다는 목표의 구체적 기준, 대표 화면·요청, 실제 컴포넌트 재사용과 코드·화면 검증 방법, 반복 실행의 일관성 및 FE 수정 작업 절감 여부를 정합니다. 전달되지 않은 특정 화면 배치까지 정확히 추론한다고 약속하지 않습니다.
5. **신규 UI와 에디터 확장:** 기존 컴포넌트 조합으로 해결되지 않을 때 질문·제안·팀 검토로 이어지는 기준을 논의합니다. AI가 임의로 새 규칙을 공식화하는 방식은 미합의입니다. 이후 에디터의 필요성, 요소 선택과 코드 연결, 제공 화면·범위·기간을 판단합니다. 기존 JSON 편집기 구조나 12~19일 추정을 새 과제에 그대로 적용하지 않습니다.
