...
- 최신 구성은 **기존 후보 링크 → 4개 주제별 11개 안건 리스트**입니다. 각 안건은 **업무명 → 대상·기간·해당하는 부분 병행 후보 → 하는 일/흐름 → 구현 조건** 순서입니다. 사용자가 넓힌 표도 가독성이 부족하다며 리스트 전환을 허용해 표와 이미지 2장을 제거했습니다. 정보는 유지하고 한 안건 안에서 읽는 순서를 통일합니다. 기간 세부 분해·긴 서문·관리용 식별자·접기·중복 해설은 넣지 않습니다.
