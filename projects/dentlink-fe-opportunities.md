...
- 최신 구성은 **기존 후보 링크 → 4개 주제별 11개 안건 리스트**입니다. 각 안건은 **(대상) 업무명 → 기간·해당하는 부분 병행 후보 → 하는 일/흐름 → 구현 조건** 순서입니다. 제목 앞에 (앱), (기공소 앱), (공통 웹), (내부 웹), (어드민 웹), (QA 도구), (치과·랩 웹)을 붙였고 아래 중복 대상 표기는 제거했습니다. 새 배분 화면·주문 생성 위치·QA 사용자는 제목 또는 본문에 보존했습니다. 사용자가 넓힌 표도 가독성이 부족하다며 리스트 전환을 허용해 표와 이미지 2장을 제거했습니다. 정보는 유지하고 한 안건 안에서 읽는 순서를 통일합니다. 기간 세부 분해·긴 서문·관리용 식별자·접기·중복 해설은 넣지 않습니다.
- **에디터에서 줄이면 안 되는 핵심:** 실제 FE 코드 재사용·작업 감소가 포인트입니다. JSON으로 무엇이 자동화되는지 설명한 뒤, 최신 요청에 따라 중복을 덜어 **만들 것 / 작동 원리 / 재사용과 추가 개발 / 검증 범위** 4개 설명으로 축약했습니다. FE가 DS·주문목록 한 종류의 화면 틀·설정 연결 코드·선택/미리보기/JSON 파일 저장·불러오기·허용값 검사 도구를 준비하고, PM·디자이너의 선택을 JSON으로 받아 같은 React 틀에 적용합니다. 자동화 대상은 준비된 UI의 구성 반영이며 새 기능 코드 생성이 아닙니다. 열 순서·문구·허용 필터는 설정으로 재사용하지만 새 승인 버튼·새 배치/필터·API 연결·권한·예외 처리는 추가 개발이 필요합니다. 에디터는 샘플 데이터, 제품은 FE가 연결한 실제 데이터/동작을 쓰며 제품 반영은 FE 검토·적용·배포를 거칩니다. 대상 DS+한 화면 재사용 검증 12~19일, AI/범용 편집기 확장안, 실제 절감 효과 미검증은 유지합니다.
...
- **사용자 삭제 존중:** 사용자가 서문·기간 기준·함께 고를 때 주석을 직접 삭제했음을 최신 Notion에서 확인했고 복원하지 않았습니다. 아래 세부 산정 근거는 개인 기록에만 유지합니다. 현재 항목에 남아 있던 에디터의 DS 포함 관계는 유지했습니다. 초록 표시는 파일별 제한 안내·컴포넌트 하나씩 정비·배송 조회 상태 표시의 3곳에만 둡니다. 사용자가 의미를 질문해 **작게 나눠 진행할 후보**로 바꾸고, 전체 안건에서 작은 부분을 떼어 다른 업무와 병행할 후보라는 설명을 위에 짧게 추가했습니다. 추가 작업·전체 안건의 병행 확정이 아니며 미표시 안건의 병행 가능 여부도 미판정입니다.
