...
- 사진 아이디어는 **주문 선택 후 사진 보내기**(기공소 앱)와 **촬영한 사진을 여러 주문에 배분**(기공소 앱·별도 배분 화면)으로 구분합니다. 첫 안은 표/이미지 모두 QR → 촬영 또는 갤러리 선택 → 확인/전송 → 해당 주문 링크톡이며, 앱에서 주문을 직접 선택하는 대안도 이미지 안에 표시했습니다. 두 번째는 새 사진 배분 화면 안에서 사진 선택·주문별 수동 지정·확인/전송을 하며 A/B 주문 배정 예시, 미배정 확인·주문별 결과 등 필요한 기능을 이미지에 담았습니다. 전자는 8~13일, 후자는 독립 업무로 10~15일입니다. 과거 15~25일은 전자까지 구현하는 누적값이므로 독립 업무의 기간으로 재사용하지 않습니다.
- 기간은 **8~13일** 형식의 총합만 표시합니다. 실패 조사+복구 6~11일, 속도 분석+FE 개선 5~10일, 디자인시스템 대상 정비+한 화면 에디터 12~19일입니다. 미산정으로 남았던 사진 배분은 10~15일, 파일 탭은 7~10일, 어드민 상세는 7~11일로 후속 정적 검토·범위 가정에 따라 보완했습니다. 확정 견적이 아니며 구현 가정·의존성과 자세한 근거는 아래 개인 기록에 유지하고 팀 문서에 반복하지 않습니다.
