...
- **에디터에서 줄이면 안 되는 핵심:** 실제 FE 코드 재사용·작업 감소가 포인트입니다. 해당 리스트의 흐름에 대상 디자인시스템 정비 → PM·디자이너의 한 화면 구성 → 설정(JSON) → 같은 React 템플릿·설정의 제품 재사용을 명시했습니다. FE는 실제 컴포넌트와 설정을 연결하는 코드, API·권한·업무 로직을 개발합니다. UI 재작성·수정 왕복 감소를 검증한다는 점을 유지했으며 기존 UI·Storybook·주문목록 기반 확인이 제품 연동·효과 입증을 뜻하지 않습니다.
