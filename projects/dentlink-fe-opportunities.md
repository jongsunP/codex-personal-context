...
- 최신 구성은 **기존 후보 링크 → 4개 주제별 11개 안건 리스트**입니다. 각 안건은 **(대상) 업무명(안건 전체의 병행 후보는 초록 제목) → 기간 → 하는 일/흐름 → 구현 조건** 순서입니다. 제목 앞에 (앱), (기공소 앱), (공통 웹), (내부 웹), (어드민 웹), (QA 도구), (치과·랩 웹)을 붙였고 아래 중복 대상 표기는 제거했습니다. 새 배분 화면·주문 생성 위치·QA 사용자는 제목 또는 본문에 보존했습니다. 사용자가 넓힌 표도 가독성이 부족하다며 리스트 전환을 허용해 표와 이미지 2장을 제거했습니다. 정보는 유지하고 한 안건 안에서 읽는 순서를 통일합니다. 기간 세부 분해·긴 서문·관리용 식별자·접기·중복 해설은 넣지 않습니다.
- **에디터에서 줄이면 안 되는 핵심:** 실제 FE 코드 재사용·작업 감소가 포인트입니다. 사용자가 ‘주문목록 1종’의 출처와 결과물을 이해하기 어렵다고 지적했고, 채팅의 추가 설명 없이 Notion 자체로 이해되어야 한다고 명확히 했습니다. 최신 항목은 **만드는 도구 / 사용 예시 / FE가 개발할 것 / 가능한 범위** 순서입니다. 목표는 PM·디자이너의 화면 구성 결과를 제품 코드에 재사용하는 내부 편집기이며, 먼저 FE가 준비한 목록 화면에서 정해진 열·필터·문구를 바꾸는 제한된 검증안을 제안합니다. **적용 화면은 미정이고 주문목록은 제가 산정을 위해 넣었던 예시이지 팀의 확정 요구사항이 아닙니다.** 대상 DS 정비+이 검증안의 12~19일은 화면 선정에 따라 조정되는 추정이며 범용 에디터 견적이 아닙니다. 미리보기와 제품에 같은 화면 코드를 쓰고 선택·설정 저장/불러오기·검사를 개발하며, 미리보기는 샘플 데이터이고 제품 데이터·권한·업무 동작은 FE가 연결합니다. 새 배치·컴포넌트·기능은 추가 개발이며 실서비스 반영은 FE 검토·적용·배포를 거칩니다. 실제 작업 감소는 검증할 사항이고 AI/자유로운 화면 제작은 확장안입니다. 아래 과거 기록의 주문목록 한 종류는 이 예시를 설명한 이력으로 읽고 확정 범위로 복원하지 않습니다.
- 사진 아이디어는 **주문 선택 후 사진 보내기**(기공소 앱)와 **촬영한 사진을 여러 주문에 배분**(기공소 앱·별도 배분 화면)으로 구분합니다. 첫 안은 QR → 로그인·주문 확인 → 촬영 또는 갤러리 → 사진 확인/전송 → 해당 주문 링크톡이며 앱에서 주문을 직접 선택하는 대안도 본문에 표시했습니다. 두 번째는 사진 선택·주문별 수동 지정·미배정/주문별 배정 확인·순차 전송/결과와 A/B 배정 예시를 본문에 담았고 QR이 필수 아님을 명시했습니다. 전자는 8~13일, 후자는 독립 업무로 10~15일입니다. 과거 15~25일은 전자까지 구현하는 누적값이므로 독립 업무의 기간으로 재사용하지 않습니다.
...
- 흐름: 식별 스티커 출력·부착 → 한 주문의 제품과 마커를 함께 촬영 → 사진 여러 장 선택 → OCR/사진 속 QR로 식별값 읽기 → 접근 가능한 주문과 대조 → 주문별 자동 분류를 사용자가 확인·수정 → 기존 첨부 제한에 맞춰 주문별 링크톡 전송. 인식 실패·복수 번호·조회 불일치는 수동 배정합니다. 제품 모양을 AI로 판별하거나 확인 없이 자동 발송하는 안이 아닙니다. 스티커를 보완한다면 읽을 수 있는 주문번호+QR 조합을 추천하며 실제 인식률은 미검증입니다.
