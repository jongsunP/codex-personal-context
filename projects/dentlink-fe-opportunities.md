...
- 새 페이지는 **요약표 1개(9개 세부 업무) → 파일·링크톡 → 디자인시스템/화면 구성 → 운영/QA 데이터 → 회의에서 좁힐 점·코드 근거** 순서입니다. 접힌 내용이나 추가 하위 페이지는 없습니다. 현황·제안·FE 범위·조건·대략 공수를 구분했습니다.
...
- 사용자가 1차 문서의 `치과나 랩의 파일탭`을 기존 파일 탭과 제안의 차이에 대한 질문으로, `어드민 주문상`을 빠른 확인을 위해 별도로 설계한 관리자 상세의 개선 의견으로 명확히 설명했습니다. Clinic/Lab과 동일하게 만들자는 뜻이 아닙니다. 이번 요청은 재검토이며 Notion·Sites·제품 코드는 변경하지 않았습니다.
...
- 파일 탐색은 기존 업로드/첨부 UX와, 관리자 상세 처리는 운영팀의 주문 생성 편의성과 구분합니다. 2차 문서에는 아직 추가하지 않았습니다. 다음 문서 반영 시 두 의견을 놓치지 않고 좁힌 범위 기준으로 공수·우선순위를 재검토합니다.
