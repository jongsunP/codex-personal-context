...
- 최신 구성은 **업무 목록 표 1개(11개 안·업무/대상/설명/예상 총 작업일) → 도식 2장**입니다. 사용자는 아이디어를 다시 고르고 대략 산정하는 단계라며 간결함과 항목 간 일관성을 요청했습니다. 이미지에 설명을 흡수해 별도 설명 4개, `참고 도식` 제목/안내, 이미지 밖 개별 제목·에디터 설명 문단도 제거했습니다. 필요한 제목·핵심 설명은 이미지 안에 넣었습니다. 작업일 기준·계산·첫 범위·판단 기준 등 구현 계획 문장을 다시 늘리지 않습니다.
- **에디터에서 줄이면 안 되는 핵심:** 실제 FE 코드 재사용·작업 감소가 포인트입니다. 표에는 PM·디자이너가 구성한 화면의 코드 재사용을, 새 도식 안에는 대상 디자인시스템 정비 → 실제 컴포넌트로 구성 → 설정(JSON) → FE 서비스 연결을 명시했습니다. 공통 UI·React 화면 템플릿·설정을 함께 사용하고 API·권한·업무 로직은 FE가 연결합니다. UI 재작성·수정 왕복을 줄이는 목표와 `구현 구조 검토 · 실제 효과 미검증`, 한 화면에서 검증할 내용을 이미지 안에 통합했습니다. 기존 UI·Storybook·주문목록 기반을 읽었지만 제품 연동이나 절감 효과가 입증된 것은 아닙니다.
- 사진 아이디어는 **주문 선택 후 사진 보내기**(기공소 앱)와 **촬영한 사진을 여러 주문에 배분**(기공소 앱·별도 배분 화면)으로 구분합니다. 첫 안은 표/이미지 모두 QR → 촬영 또는 갤러리 선택 → 확인/전송 → 해당 주문 링크톡이며, 앱에서 주문을 직접 선택하는 대안도 이미지 안에 표시했습니다. 두 번째는 새 사진 배분 화면 안에서 사진 선택·주문별 수동 지정·확인/전송을 하며 A/B 주문 배정 예시, 미배정 확인·주문별 결과 등 필요한 기능을 이미지에 담았습니다. 전자는 8~13일, 후자는 미산정입니다. 과거 15~25일은 전자까지 구현하는 누적값이므로 독립 업무의 기간으로 재사용하지 않습니다.
...
- 도식은 **photo-flows-v2.png(1500×1140)**, **editor-flow-v2.png(1500×1000)**로 교체해 Notion 자체 저장소에 첨부했습니다. 임시 SVG·생성 스크립트는 `/tmp/dentlink-fe-second-meeting/`의 v2 파일이며 팀 공유 원본은 Notion 첨부입니다. 원본·720px 축소본·실제 Notion 표시를 확인했습니다. 기존 PNG와 이미지 밖 중복 설명은 현재 페이지에서 제거했습니다.
...
- 최종 검증: 표 11개 안·4열·새 이미지 2개, QR/갤러리 흐름·별도 배분 화면·에디터의 FE 재사용 가치 반영, 중복 본문 제거를 재조회했습니다. 두 이미지가 실제 Notion에 표시되고 한글·흐름·배정 예시·검증 상태가 읽히는 것을 확인했습니다. 서명된 기존 이미지 URL 때문에 부분 치환이 실패해 최신 본문을 다시 읽고 표를 그대로 보존한 전체 본문 교체로 완료했습니다. 1차 Notion·Sites·제품 코드는 변경하지 않았습니다.
