...
- **버전 관계 확정:** 사용자가 **DLDS=v1, 공유한 DLOS 페이지=v2**라고 명확히 했습니다. **v2처럼 정리된 사례를 참고해 v1의 기존 코드를 정비하고 컴포넌트를 확인할 수 있게 정리**하려는 작업입니다. 두 버전을 완전히 동일하게 만들거나 v1을 v2로 전면 이전하는 요구가 아닙니다. 기존 구현의 불일치·props·기본값·사용 방법을 정리하는 범위부터 구체화합니다. 버전 관계를 다시 미확정 질문으로 되돌리지 않습니다.
...
1. **v1 정비 범위:** 먼저 다룰 실제 컴포넌트와 사용 화면, props·기본값·불일치 정리 기준, 기존 사용처에 영향을 주는 변경 범위를 정합니다. 다음 논의는 이 범위와 정리 완료 기준부터 시작합니다.
