...
- 최신 구성은 **짧은 회의 목적 → 업무 표 1개(11개 안·업무/대상 묶음, 하는 일/범위, FE 구현/추가 고려, 예상 작업일) → 공통 주석 2개 → 도식 2장**입니다. 처음에는 설명을 줄였으나 사용자는 최종적으로 ‘무조건 간단하게’가 아니라 FE 동료가 문서만 보고 2차 회의에서 후보를 비교·선별할 수 있도록 요청했습니다. 각 행에 범위·FE/서버 경계를 짧게 넣고 아래에 11개 설명을 다시 반복하지 않았습니다. 기간 세부 분해·긴 서문·관리용 식별자·접기·중복 해설은 넣지 않습니다.
...
- 공통 주석은 FE 1명의 분석·개발·개발자 QA 초기 추정, 실제 관측·서버 작업·협업 대기 제외, 실연동·효과 미검증과 포함 관계만 간결히 설명합니다. 에디터는 대상 DS 정비, 주문 선택 후 사진 보내기는 첨부 UX를 포함합니다. 계측·전송 등 공통 작업은 단순 합산하지 않습니다. 확장 에디터 추가 12~20일은 범위 미선정으로 팀 문서에 넣지 않으며 앞선 개인 추정치를 확정 견적으로 되살리지 않습니다.
